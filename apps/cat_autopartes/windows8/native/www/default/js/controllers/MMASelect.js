...
function setAnios(pInicio, pFin){
	$('#selAnio option[value!="0"]').remove();
	
	for(pInicio; pInicio <= pFin; pInicio++){
		$('#selAnio').append('<option value="' + pInicio + '">' + pInicio + '</option>');
	}
...
function goToFamilias(){
	...
	if($('#selMarca').val() != '0' && $('#selModelo').val() != '0'){
		...
		$('#lblAutoAnio, #lblAutoAnioProd').text($('#selAnio').val());
	}
	else{
		//TODO Message
	}
